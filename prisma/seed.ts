...
type VerifiedOverallResultSeed = {
  eventSlug: string;
  riderSlug: string;
...
const ERZBERGRODEO_2026_SOURCE_NOTE =
  "Verified/manual/source-tracked first-pass placeholder: Red Bull Erzbergrodeo 2026 overall podium verified as P1 Manuel Lettenbichler, P2 Trystan Hart, P3 Mario Roman; 15 riders reached the finish; Billy Bolt is not listed as a finisher. Exact timing, gaps, penalties, and points are not verified in this seed and remain empty.";

const verifiedOverallResults: VerifiedOverallResultSeed[] = [
  {
    eventSlug: "erzbergrodeo-2026",
    riderSlug: "manuel-lettenbichler",
    overallPosition: 1,
    status: "FINISHED",
    sourceNote: ERZBERGRODEO_2026_SOURCE_NOTE,
  },
  {
    eventSlug: "erzbergrodeo-2026",
    riderSlug: "trystan-hart",
    overallPosition: 2,
    status: "FINISHED",
    sourceNote: ERZBERGRODEO_2026_SOURCE_NOTE,
  },
  ...
  for (const seed of eventSeeds) {
    ...
    const event = await prisma.event.create({
      data: {
        ...
        countryId: countryMap.get(seed.countrySlug)?.id,
        name:
          seed.slug === "erzbergrodeo-2026" ? "Red Bull Erzbergrodeo 2026" : seed.name,
        slug: seed.slug,
...
function getEventDescription(seed: EventSeed) {
  ...
  if (seed.slug !== "erzbergrodeo-2026") {
    return base;
  }

  return `${base} Previous winner marker: Manuel Lettenbichler. Verified winner marker: Manuel Lettenbichler. Verified finisher count: 15. Verified result source note: ${ERZBERGRODEO_2026_SOURCE_NOTE}`;
}
  ...
  for (const seed of verifiedOverallResults) {
    ...
    await prisma.result.create({
      data: {
        ...
        classPosition: seed.overallPosition,
        points: null,
        totalTimeMs: null,
        totalTimeText: null,
        gapToLeaderMs: null,
        ...
        status: seed.status,
        notes: seed.sourceNote,
        officialRawRow: {
          verified: true,
          sourceTracking: "manual/source-tracked-placeholder",
          event: "Red Bull Erzbergrodeo 2026",
          resultType: "overall",
          verifiedFacts: {
            ...
            status: seed.status,
            finishers: 15,
            nonFinisherNote: "Billy Bolt is not listed as a finisher.",
          },
          unknownFields: [
            "totalTimeMs",
            "totalTimeText",
...
function getVerifiedCareerStats(riderSlug: string) {
  ...
  return {
    starts: 1,
    wins: result.overallPosition === 1 ? 1 : 0,
    podiums: result.overallPosition <= 3 ? 1 : 0,
    achievements: [
      `Verified first-pass Red Bull Erzbergrodeo 2026 overall P${result.overallPosition}`,
    ],
  };
}
...
async function seedSourcesAndAudit(
  ...
  const [
    ...
    prisma.sourceSnapshot.create({
      data: {
        ...
        contentHash: "verified-erzbergrodeo-2026-first-pass-001",
        rawContent: ERZBERGRODEO_2026_SOURCE_NOTE,
        fetchedAt: new Date("2026-06-08T08:00:00.000Z"),
        statusCode: 200,
      },
    }),
    prisma.sourceSnapshot.create({
  ...
  const verifiedErzbergResults = await prisma.result.findMany({
    where: {
      event: {
  ...
  for (const result of verifiedErzbergResults) {
    await prisma.sourceLink.create({
      data: {
        dataSourceId: fimSource.id,
        url: "https://example.com/source-placeholder/red-bull-erzbergrodeo-2026-results",
        entityType: "RESULT",
        entityId: result.id,
        note: `${ERZBERGRODEO_2026_SOURCE_NOTE} Linked result row: P${result.overallPosition} ${result.rider.firstName} ${result.rider.lastName}.`,
      },
    });
  }
  ...
  const verifiedResultImportRun = await prisma.importRun.create({
    data: {
      ...
      jobName: "manual-verified-erzbergrodeo-2026-results",
      status: "COMPLETED",
      startedAt: new Date("2026-06-08T08:01:00.000Z"),
      finishedAt: new Date("2026-06-08T08:02:00.000Z"),
      recordsFound: verifiedErzbergResults.length,
      recordsCreated: verifiedErzbergResults.length,
      recordsUpdated: 0,
      recordsSkipped: 0,
      metadata: {
        verified: true,
        ...
        note: ERZBERGRODEO_2026_SOURCE_NOTE,
      },
    },
  });
  ...
  for (const result of verifiedErzbergResults) {
    await prisma.dataVersion.create({
      data: {
        ...
        sourceUrl:
          "https://example.com/source-placeholder/red-bull-erzbergrodeo-2026-results",
