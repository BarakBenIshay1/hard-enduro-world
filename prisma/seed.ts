...
import { verifiedOverallResults } from "../data/verified/results";
import { getVerifiedSource, verifiedSources } from "../data/verified/sources";
import type { VerifiedOverallResult } from "../data/verified/types";
...
async function main() {
  ...
  for (const seed of eventSeeds) {
    ...
    const event = await prisma.event.create({
      data: {
        seasonId: season.id,
        countryId: countryMap.get(seed.countrySlug)?.id,
        name: getVerifiedEventFact(seed.slug)?.displayName ?? seed.name,
        slug: seed.slug,
        ...
        description: getEventDescription(seed),
      },
    });
  ...
  await seedVerifiedOverallResults(eventMap);
  await seedMembershipsAndCareers();
  await seedSourcesAndAudit(eventMap);

  console.log(
    `Seed complete: ${eventSeeds.length} events, ${riders.length} rider profile placeholders, ${verifiedOverallResults.length} verified Erzbergrodeo 2026 podium rows, no synthetic historical results.`,
  );
}

function getEventDescription(seed: EventSeed) {
  const base = `${DEMO_NOTE} Event metadata placeholder only. Terrain: ${seed.terrain}. Elevation: ${seed.elevation}. No fictional winner, podium, points, standings, or timing data is generated by seed data.`;
  const verifiedFact = getVerifiedEventFact(seed.slug);
  ...
  return [
    base,
    verifiedFact.previousWinner
      ? `Previous winner marker: ${verifiedFact.previousWinner}.`
      : null,
    verifiedFact.verifiedWinner
      ? `Verified winner marker: ${verifiedFact.verifiedWinner}.`
      : null,
    verifiedFact.verifiedFinisherCount !== null
      ? `Verified finisher count: ${verifiedFact.verifiedFinisherCount}.`
      : null,
    `Verified result source note: ${verifiedFact.factsNote}`,
  ]
    .filter(Boolean)
    .join(" ");
}
...
async function seedVerifiedOverallResults(
  eventMap: Map<string, { id: string; slug: string; name: string }>,
) {
  for (const seed of verifiedOverallResults) {
    const event = eventMap.get(seed.eventSlug);
    const verifiedFact = getVerifiedEventFact(seed.eventSlug);
    const rider = riders.find((item) => item.slug === seed.riderSlug);
    const riderRecord = prismaRiderMap.get(seed.riderSlug);

    if (!event || !rider || !riderRecord) {
      continue;
    }
    ...
    await prisma.result.create({
      data: {
        ...
        overallPosition: seed.overallPosition,
        classPosition: seed.classPosition,
        points: seed.points,
        totalTimeMs: seed.totalTimeMs,
        totalTimeText: seed.totalTimeText,
        gapToLeaderMs: seed.gapToLeaderMs,
        gapToLeaderText: seed.gapToLeaderText,
        gapToPreviousMs: seed.gapToPreviousMs,
        gapToPreviousText: seed.gapToPreviousText,
        penaltiesMs: seed.penaltiesMs,
        bonusTimeMs: seed.bonusTimeMs,
        checkpointsCompleted: seed.checkpointsCompleted,
        averageSpeedKmh: seed.averageSpeedKmh,
        status: seed.status,
        notes: buildVerifiedResultNote(seed),
        officialRawRow: {
          verified: true,
          sourceTracking: "manual/source-tracked-placeholder",
          event: event.name,
          resultType: "overall",
          verifiedFacts: {
            position: seed.overallPosition,
            rider: riderName(rider),
            status: seed.status,
            finishers: verifiedFact?.verifiedFinisherCount,
          },
          sources: seed.sourceIds
            .map((sourceId) => getVerifiedSource(sourceId))
          ...
          unknownFields: [
            ...
            "penaltiesMs",
            "points",
          ].filter((field) => seed[field as keyof VerifiedOverallResult] === null),
          note: buildVerifiedResultNote(seed),
        },
      },
    });
  }
...
function buildVerifiedResultNote(result: VerifiedOverallResult) {
  const sourceNotes = result.sourceIds
    .map((sourceId) => getVerifiedSource(sourceId))
    .filter((source) => source !== null)
    .map(
      (source) =>
        `${source.name}; confidence=${source.confidence}; url=${source.url}; published=${source.publishedDate ?? "unknown"}`,
    );

  return [result.notes, ...sourceNotes].join(" ");
}
...
async function seedMembershipsAndCareers() {
  ...
  for (const rider of riders) {
    ...
    const manufacturer = prismaManufacturerMap.get(rider.manufacturerSlug)!;
    const motorcycle = prismaMotorcycleMap.get(rider.motorcycleSlug)!;
    const verifiedCareerStats = getVerifiedCareerStats(rider.slug);
    ...
    await prisma.riderCareerSeason.create({
      data: {
        ...
        points: 0,
        wins: verifiedCareerStats.wins,
        podiums: verifiedCareerStats.podiums,
        starts: verifiedCareerStats.starts,
        dnfs: 0,
        stageWins: 0,
        ...
        statistics: {
          ...
          note:
            verifiedCareerStats.starts > 0
              ? "Profile placeholder with only verified Erzbergrodeo 2026 first-pass result totals applied. Points, standings, timing, and broader career history remain unverified and empty."
              : "Profile placeholder only. No historical starts, points, wins, podiums, standings, or results are generated.",
          ...
          achievements: verifiedCareerStats.achievements,
          status:
            verifiedCareerStats.starts > 0
              ? "profile placeholder with verified Erzbergrodeo 2026 result"
              : "profile placeholder",
...
function getVerifiedCareerStats(riderSlug: string) {
  const result = verifiedOverallResults.find((item) => item.riderSlug === riderSlug);
  ...
  const verifiedPosition = result.overallPosition;

  return {
    starts: 1,
    wins: verifiedPosition === 1 ? 1 : 0,
    podiums: verifiedPosition !== null && verifiedPosition <= 3 ? 1 : 0,
    achievements: [
      verifiedPosition
        ? `Verified first-pass ${result.eventSlug} overall P${verifiedPosition}`
        : `Verified first-pass ${result.eventSlug} result row`,
    ],
  };
}
...
async function seedSourcesAndAudit(
  ...
  const verifiedSourceSnapshots = new Map<string, { id: string }>();

  for (const source of verifiedSources) {
    const dataSource =
      source.sourceType === "manual_verified"
        ? manualSource
        : source.sourceType === "timing_system"
          ? timingSource
          : fimSource;
    const snapshot = await prisma.sourceSnapshot.create({
      data: {
        ...
        url: source.url,
        contentHash: `${source.id}-snapshot`,
        rawContent: source.notes,
        fetchedAt: new Date(
          source.publishedDate
            ? `${source.publishedDate}T08:00:00.000Z`
            : "2026-06-08T08:00:00.000Z",
        ),
        statusCode: 200,
      },
    });
    verifiedSourceSnapshots.set(source.id, snapshot);
  }
  ...
  const verifiedSeedResults = await prisma.result.findMany({
    where: {
      event: {
        slug: {
          in: Array.from(
            new Set(verifiedOverallResults.map((result) => result.eventSlug)),
          ),
        },
      },
    },
  ...
  for (const result of verifiedSeedResults) {
    const resultSourceId =
      verifiedOverallResults.find((item) => item.riderSlug === result.rider.slug)
        ?.sourceIds[0] ?? verifiedSources[0]?.id;
    const resultSource = resultSourceId ? getVerifiedSource(resultSourceId) : null;

    await prisma.sourceLink.create({
      data: {
        dataSourceId:
          resultSource?.sourceType === "manual_verified" ? manualSource.id : fimSource.id,
        url:
          resultSource?.url ?? "https://example.com/source-placeholder/verified-result",
        entityType: "RESULT",
        entityId: result.id,
        note: `${resultSource?.notes ?? "Verified source note pending."} Linked result row: P${result.overallPosition} ${result.rider.firstName} ${result.rider.lastName}.`,
      },
    });
  }

  const primaryVerifiedSource = verifiedSources[0];

  const verifiedResultImportRun = await prisma.importRun.create({
    data: {
      sourceSnapshotId: primaryVerifiedSource
        ? verifiedSourceSnapshots.get(primaryVerifiedSource.id)?.id
        : undefined,
      jobName: "manual-verified-results-seed",
      status: "COMPLETED",
      startedAt: new Date("2026-06-08T08:01:00.000Z"),
      finishedAt: new Date("2026-06-08T08:02:00.000Z"),
      recordsFound: verifiedSeedResults.length,
      recordsCreated: verifiedSeedResults.length,
      recordsUpdated: 0,
      recordsSkipped: 0,
      metadata: {
        verified: true,
        manualFirstPass: true,
        eventSlugs: Array.from(
          new Set(verifiedOverallResults.map((result) => result.eventSlug)),
        ),
        reviewRequiredForFutureChanges: true,
  ...
  for (const result of verifiedSeedResults) {
    await prisma.dataVersion.create({
      data: {
        entityType: "RESULT",
        entityId: result.id,
        importRunId: verifiedResultImportRun.id,
        action: "CREATE",
        next: {
        ...
        sourceUrl: primaryVerifiedSource?.url,
        createdBy: "manual-seed-verification",
      },
    });
  }
