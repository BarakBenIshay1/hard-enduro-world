...
export default async function AdminResultsPage({ searchParams }: PageProps) {
  ...
  const data = await getAdminResults(filters);
    ...
    <div className="grid min-w-0 gap-6">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black lg:text-5xl">Overall Results CMS</h1>
          <p className="mt-4 max-w-3xl text-sm leading-6 text-foreground/[0.62]">
            Manage persisted overall classifications. Imports, reviews, source lineage,
            and public page design remain separate from this manual correction layer.
          </p>
        </div>
        <AdminStatusBadge status="ready" />
      </section>

      <ResultMessage code={value(params, "error") ?? value(params, "saved")} />

      <Card className="p-5">
        <form className="grid gap-4 lg:grid-cols-[1.2fr_1fr_1fr_0.8fr_1fr_1fr_1fr_1fr_1fr_1fr_auto] lg:items-end">
          <Field label="Search">
            <input
              name="search"
              defaultValue={filters.search ?? ""}
              placeholder="Event, rider, class"
          ...
          >
            <option value="">All</option>
            {data.options.manufacturers.map((manufacturer) => (
          ...
          >
            <option value="">All</option>
            {data.options.motorcycles.map((motorcycle) => (
          ...
          <Select name="sort" label="Sort" value={filters.sort ?? "event-desc"}>
      ...
      <Card className={adminTableCardClass}>
        <div className="border-b border-border p-5">
          <h2 className="text-xl font-black">Overall classifications</h2>
          <p className="mt-2 text-sm text-foreground/[0.62]">
            {data.total} results found. Page {data.page} of {data.totalPages}.
          </p>
        </div>
        <div className={adminTableScrollClass}>
          <table className={adminCompactTableClass}>
            <thead className={adminTableHeadClass}>
              <tr>
                <th className={adminTablePrimaryCellClass}>Result</th>
            ...
            <tbody>
              {data.results.map((result) => (
                <tr key={result.id} className="border-t border-border">
                  ...
                  <td className={adminTableCellClass}>{result.event.name}</td>
        ...
        {data.results.length === 0 ? (
          <div className="border-t border-border p-6 text-sm text-foreground/[0.62]">
            No overall results match the current filters.
          </div>
        ) : null}
      </Card>
...
function parseFilters(
  ...
  return {
    ...
    manufacturerId: value(params, "manufacturerId"),
    motorcycleId: value(params, "motorcycleId"),
    ...
    sort: value(params, "sort") ?? "event-desc",
  };
}
...
function PageLink({
  ...
  return disabled ? (
    ...
    <Link
      href={`/admin/results?${params.toString()}`}
...
function ResultMessage({ code }: { code?: string }) {
  if (!code) return null;
  const messages: Record<string, string> = {
    updated: "Result saved.",
    archived: "Result archived.",
    restored: "Result restored.",
    unauthorized: "You do not have permission to manage results.",
    "archive-confirmation": "Confirm archive before submitting.",
    "restore-confirmation": "Confirm restore before submitting.",
    "duplicate-result": "A result already exists for this event, rider, and class.",
    ...
    "archived-locked": "Restore the result before editing it.",
    "database-unavailable": "The result could not be saved. Please try again.",
  };
  return (
