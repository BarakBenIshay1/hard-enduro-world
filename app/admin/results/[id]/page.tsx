...
import { getAdminResultDetail, getAdminResultOptions } from "@/db/admin-results";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const result = await getAdminResultDetail(id);
  return {
    title: result
      ? `${result.rider.firstName} ${result.rider.lastName} result`
      : "Result",
    robots: { index: false, follow: false, nocache: true },
  };
}

export default async function AdminResultDetailPage({ params, searchParams }: PageProps) {
  ...
  const [result, classification, classificationHistory] = await Promise.all([
    getAdminResultDetail(id),
    resolveRecordClassification(ClassifiableEntityType.RESULT, id),
    getRecordClassificationHistoryWithEvidence(ClassifiableEntityType.RESULT, id),
  ]);
  if (!result) notFound();
  ...
  return (
    <div className="grid min-w-0 gap-6 overflow-x-hidden">
      <section className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          ...
          <h1 className="mt-3 break-words text-3xl font-black lg:text-4xl">
            {riderName} · {result.event.name}
          </h1>
          <p className="mt-4 max-w-3xl text-sm leading-6 text-foreground/[0.62]">
            Edit persisted overall classification fields only. Source lineage, review
            history, and importer metadata remain attached.
          </p>
        </div>
      ...
      <ResultMessage code={query?.error ?? query?.saved} />

      <div className="grid min-w-0 gap-5 2xl:grid-cols-[minmax(0,1fr)_320px] 2xl:items-start">
        <Card className="min-w-0 p-4 sm:p-5">
          <h2 className="text-xl font-black">Result editor</h2>
          ...
          <EventEditorForm action={updateAdminResult} className="mt-5 grid gap-4">
            <input type="hidden" name="resultId" value={result.id} />
            <div className="grid min-w-0 gap-3 lg:grid-cols-2">
              <ReadOnlyField label="Event" value={result.event.name} />
        ...
        <aside className="grid min-w-0 gap-4 self-start 2xl:sticky 2xl:top-6">
          <Card className="min-w-0 p-4">
            ...
            <div className="mt-5 grid gap-3 text-sm">
              ...
              <Meta label="Event" value={result.event.name} />
            ...
            <div className="mt-4 grid gap-3">
              {result.sourceLinks.length ? (
                ...
                <p className="text-sm text-foreground/[0.58]">
                  No source links are attached to this result.
                </p>
              )}
            </div>
          </Card>
          ...
          <Card className="min-w-0 p-4">
            ...
            {canManage && !result.archivedAt ? (
              <form action={archiveAdminResult} className="mt-5 grid gap-3">
                <input type="hidden" name="resultId" value={result.id} />
              ...
              <form action={restoreAdminResult} className="mt-5 grid gap-3">
                <input type="hidden" name="resultId" value={result.id} />
                <label className="flex items-center gap-2 text-sm">
              ...
              <p className="mt-4 text-sm text-foreground/[0.58]">
                {result.archivedAt ? "This result is archived." : "Read-only access."}
              </p>
            )}
          </Card>
        </aside>
