...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!motorcycleId) return uploadError("missing-entity-id", 400);
    if (!isSafeAdminMediaEntityId(motorcycleId)) {
    ...
    if (!motorcycle) return uploadError("invalid-entity-id", 404);
    ...
    if (error) {
      console.error("Motorcycle hero upload failed", sanitizeAdminError(error));
    ...
    console.error("Motorcycle hero upload route failed", sanitizeAdminError(error));
    return uploadError("upload-failed", 500);
  }
}
