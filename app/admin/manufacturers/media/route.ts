...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!manufacturerId) return uploadError("missing-entity-id", 400);
    if (!isSafeAdminMediaEntityId(manufacturerId)) {
    ...
    if (!manufacturer) return uploadError("invalid-entity-id", 404);
    ...
    if (error) {
      console.error("Manufacturer logo upload failed", sanitizeAdminError(error));
    ...
    console.error("Manufacturer logo upload route failed", sanitizeAdminError(error));
    return uploadError("upload-failed", 500);
  }
}
